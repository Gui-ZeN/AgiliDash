  ...
  if (requireAdmin && !isAdmin) {
    return (
      <div className="min-h-screen bg-slate-100 dark:bg-slate-900 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-slate-800 rounded-xl p-8 max-w-md w-full text-center shadow-md">
          ...
          <p className="text-sm text-slate-400 dark:text-slate-500 mb-6">
            Logado como:{' '}
            <span className="font-medium text-slate-600 dark:text-slate-300">{user?.nome}</span>
            <br />
            Perfil:{' '}
            <span className="font-medium text-slate-600 dark:text-slate-300">{user?.perfil}</span>
          </p>
          <a
            href="/dashboard"
